...
fn test_persistent_storage_mode() {
    ...
    let tcp_port = 55250 + (std::process::id() % 100) as u16;
...
#[tokio::test]
async fn test_daemon_mode_functionality() {
    use tokio_postgres::NoTls;

    let data_dir = format!("memory://daemon_test_{}", std::process::id());
    let tcp_port = 55400 + (std::process::id() % 100) as u16;

    println!("=== Starting daemon mode test ===");
    println!("Data directory: {}", data_dir);
    println!("TCP port: {}", tcp_port);

    // Start daemon process
    let exe_dir = std::env::current_exe().expect("Failed to get current exe");
    let target_dir = exe_dir.parent().unwrap().parent().unwrap();
    ...
    println!("Starting daemon: {:?}", binary_path);

    let mut daemon_process = std::process::Command::new(&binary_path)
        .args([&data_dir, &tcp_port.to_string(), "--daemon"])
    ...
    // Wait for ready signal (from parent process before it exits)
    let stdout = daemon_process.stdout.take().expect("Failed to get stdout");
    let reader = BufReader::new(stdout);
    ...
    for line in reader.lines() {
        if start.elapsed() > Duration::from_secs(75) {
            panic!("Timeout waiting for daemon ready signal");
        }

        if let Ok(line) = line {
            println!("Daemon stdout: {}", line);
            if line.contains("\"id\":\"ready\"") && line.contains("\"success\":true") {
                ready_received = true;
                break;
            }
    ...
    // Give the child process time to start and bind the port
    tokio::time::sleep(Duration::from_millis(500)).await;
    ...
    for attempt in 1..=10 {
        match tokio_postgres::connect(&connection_string, NoTls).await {
            Ok(conn) => {
                println!("Connected to daemon on attempt {}", attempt);
                client_conn = Some(conn);
                break;
            }
            Err(e) => {
    ...
    let (client, connection) = client_conn.expect("Failed to connect to daemon");
    ...
    client
        .execute("INSERT INTO daemon_test (value) VALUES ('daemon')", &[])
        .await
        .expect("Failed to insert data");

    let rows = client
        .query("SELECT value FROM daemon_test", &[])
        .await
        .expect("Failed to query data");

    assert_eq!(rows.len(), 1, "Should have 1 row");
    let value: &str = rows[0].get("value");
    assert_eq!(value, "daemon");

    println!("Daemon functionality verified successfully");

    // Cleanup connection
    drop(client);

    // Find and kill the daemon child process using the port
    println!("Cleaning up daemon process on port {}", tcp_port);
    kill_process_on_port(tcp_port);

    println!("=== Test passed: Daemon mode functionality works ===");
}
...
fn kill_process_on_port(port: u16) {
    ...
    // Use lsof to find the process LISTENING on the port (not just connected)
    // -iTCP:LISTEN filters to show only listening TCP sockets
    let output = Command::new("lsof")
        .args(["-ti", "-iTCP:LISTEN", &format!(":{}", port)])
        .output();
        ...
        if output.status.success() {
            ...
            for pid in pids.lines() {
                if let Ok(pid_num) = pid.trim().parse::<u32>() {
                    ...
                    if pid_num != test_pid {
                        println!("Killing daemon PID: {} (test PID: {})", pid_num, test_pid);
                        let _ = Command::new("kill")
                            .args(["-9", &pid_num.to_string()])
                            .output();
                    } else {
                        println!("Skipping test PID: {}", pid_num);
                    }
                }
            }
        } else {
            eprintln!("lsof failed or no process found listening on port {}", port);
        }
    } else {
        eprintln!("Failed to run lsof to find process on port {}", port);
    }
